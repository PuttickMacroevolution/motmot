...
<p>We can summarise the analyses using <code>summary</code> and plotting the shifts on the phylogeny using the function <code>plot.traitMedusa.model</code>. These results show a decrease at node 39 that we can visualise on the phylogeny.</p>
<pre class="r"><code>trait.medusa.tm2.summary &lt;- summary(tm2.ml, cutoff = 2,
    AICc = TRUE)
...
##
## Rooted; includes branch lengths.</code></pre>
<pre class="r"><code>colour_motmot &lt;- plot.traitMedusa.model(phy = phy.clade, traitMedusaObject = trait.medusa.tm2.summary,
...
<pre class="r"><code>summary(tm2.ml, cutoff = 5.698198, AICc = TRUE)$Rates</code></pre>
...
<pre class="r"><code>outputSummary &lt;- summary(timeSlice.ml, cutoff = 1, cex = 0.2,
