  ...
  switch(model,
    ...
    ou = {
      ...
      if (!is.ultrametric(phy)) {
        ...
        vcv.matrix <- vcv(phy)
       	alpha[n.par + c(1, 2)] <- 1
       	lowerBound[n.par + (1:2)] <- c(NA, 1e-8)
      ...
      if (!is.ultrametric(phy)) {
      	...
      	brVar <- (vo$par[2 + n.par] * Ntip(phy)) / (Ntip(phy) - 1)
      	vcv.matrix <- vcv(lambdaPhy)
      	vo$value <- transformPhylo.ll(y = y, phy = lambdaPhy, alpha = vo$par[1], nodeIDs = nodeIDs, model = "OU", meserr = meserr, covPIC = covPIC, cophenetic.dist = cophenetic.dist, vcv.matrix = vcv.matrix,  ancestral.state=ancestral.state, brVar=brVar, return.reml=F)[[2]]
         }
    ...
    trend = {
      ...
      bm.model.var <- transformPhylo.ML(y, phy, model = "bm")$brownianVariance[1, 1]
      phy <- reorder(phy)
      tip.age.from.root <- c(nodeTimes(phy)[1, 1] - nodeTimes(phy)[which(phy$edge[, 2] <= Ntip(phy)), 2])
      yy <- data.frame(y, tip.age.from.root)
      if (lambdaEst == FALSE) {
        estimates <- pic.pgls(y ~ tip.age.from.root, phy, y = yy, lambda = 1, return.intercept.stat = TRUE)
      } else {
        estimates <- pic.pgls(y ~ tip.age.from.root, phy, y = yy, lambda = "ML", return.intercept.stat = TRUE)
      }
    ...
    timeslice = {
      ...
      if (is.null(splitTime)) {
      	if((maxTime - boundaryAge) < boundaryAge) boundaryAge <- testAge <- maxTime * 0.1
